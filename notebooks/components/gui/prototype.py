...
from ipywidgets import (
    ...
    FloatSlider, IntSlider, HBox, ButtonStyle, IntProgress, interact, interactive, fixed, interact_manual
)
from functools import partial
from IPython.display import (Audio, display, clear_output)
from dotmap import DotMap
from utils.structures import Sound
import utils.hpsMorph as hps
get_ipython().run_line_magic('matplotlib', 'widget')  # ipympl
get_ipython().run_cell_magic('html', '',
                             '<style>.widget-readout { color: white; font-size: 1.2em; } </style>')  # style
...
def generate_morph(gui, sound_1, sound_2, sound_morph):
    ...
    try:
        ...
        hps.hpsMorphFrame(hfreq1, hmag1, stocEnv1, hfreq2,
                          hmag2, stocEnv2, hfreqIntp, hmagIntp, stocIntp)
        # hps.transformation_synthesis(sound_1, sound_2, sound_morph, MORPHINGS_OUTPUT_FOLDER)

        #with Output(): Audio(sound_morph.synthesis.output.values.y, rate=sound_morph.fs, autoplay=True)
...
def generate_gui_components(gui, sound_1, sound_2, sound_morph):
    ...
    sound_1.play_input_file_button = Button(
        ...
        button_style='',
        layout=Layout(
            width='auto', grid_area='sound_1_play_input_file_button'),
    )
    ...
    sound_2.play_input_file_button = Button(
        ...
        button_style='',
        layout=Layout(
            width='auto', grid_area='sound_2_play_input_file_button'),
    )
    ...
    gui.morph_harmonic_frequencies_label = Label(
        ...
        layout=Layout(
            width='auto', grid_area='morph_harmonic_frequencies_label')
    )
    ...
    gui.morph_harmonic_magnitudes_label = Label(
        ...
        layout=Layout(
            width='auto', grid_area='morph_harmonic_magnitudes_label')
    )
    ...
    gui.morph_stochastic_component_label = Label(
        ...
        layout=Layout(
            width='auto', grid_area='morph_stochastic_component_label')
    )
    ...
    gui.morph_message = Button(
        description="",
        button_style='danger',
        layout=Layout(width='auto', visibility='hidden',
                      grid_area='morph_message')
    )
    ...
    sound_morph.synthesis.output.play_button = Button(
        description="Play",
        layout=Layout(width='auto', visibility='hidden',
                      grid_area='sound_morph_play_button'),
    )
    ...
    gui.results_display_header = HTML(
        ...
        layout=Layout(width='auto', visibility='hidden',
                      grid_area='results_display_header')
    )
...
def display_gui(gui, sound_1, sound_2, sound_morph):
    ...
    results_player_panel = GridBox(
        children=[
            gui.player_header, sound_morph.synthesis.output.play_button
        ],
        layout=Layout(
            width='auto',
